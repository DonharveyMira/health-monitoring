...
def login():  
    ...
    entryUsn = username_entry.get().lower()
    entryPass = password_entry.get()

    if (entryUsn == username and entryPass == password):
        message_label.config(text="Login successful!", fg="#497687")
        openHomepage()

    elif((entryUsn != username or entryPass != password) and count<3):
        count=count+1
        if(count!=2):
            message_label.config(text=f'Login failed. You have maximum 2 attempts', fg="#b12a2a")
        if(count!=1):
            message_label.config(text=f'Login failed. You have maximum 1 attempts', fg="#b12a2a")
        if(count==3):
            message_label.config(text="Maximum login attempts exceeded!", fg="#b12a2a")
            disabledTxt() 
